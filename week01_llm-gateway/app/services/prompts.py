...
class PromptRepository:
    """Stores versioned prompt templates and renders them with a Jinja2 sandbox."""
    ...
    async def initialize(self) -> None:
        async with aiosqlite.connect(self.database_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_prompts_active ON prompts(id, is_active)"
            )
            await db.commit()
    ...
    async def get(self, prompt_id: str, version: int | None = None) -> PromptRecord:
        ...
        if row is None:
            label = version or "active"
            raise GatewayError(
                f"Prompt {prompt_id!r} version {label!r} was not found",
                status_code=404,
