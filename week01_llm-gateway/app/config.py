...
ProtocolName = Literal["chat", "responses", "messages"]
PROTOCOL_VALUES: frozenset[str] = frozenset({"chat", "responses", "messages"})
ALL_PROTOCOL_VALUE = "all"
...
def parse_protocols(value: str) -> set[str]:
    ...
    if ALL_PROTOCOL_VALUE in raw_items:
        if len(raw_items) != 1:
            raise ValueError("api 'all' is reserved and cannot be combined with other protocols")
        return set(PROTOCOL_VALUES)
    unknown = set(raw_items) - PROTOCOL_VALUES
    if unknown:
        raise ValueError(f"Unknown api protocol values: {sorted(unknown)}")
    return set(raw_items)


class RetryConfig(BaseModel):
    max_retries_per_route: int = Field(default=3, ge=0, le=5)
    base_delay_seconds: float = Field(default=0.25, ge=0, le=10)
    max_delay_seconds: float = Field(default=4, ge=0, le=60)
    retry_statuses: set[int] = Field(default_factory=lambda: {408, 409, 429, 500, 502, 503, 504})

    @model_validator(mode="after")
    def validate_delay_window(self) -> RetryConfig:
        if self.max_delay_seconds < self.base_delay_seconds:
            raise ValueError("retry.max_delay_seconds must be >= retry.base_delay_seconds")
        return self
...
class RouteTarget(BaseModel):
    ...
    def protocols(self) -> set[str]:
        return parse_protocols(self.api)
...
def load_config(path: str | Path | None = None) -> GatewayConfig:
    ...
    if not config_path.exists():
        if explicit_path or "GATEWAY_CONFIG" in os.environ:
